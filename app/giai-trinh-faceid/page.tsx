...
export default function GiaiTrinhFaceIdPage() {
  ...
  const displayedOrders = useMemo(
    () => flattenedOrders.slice(0, pagination.limit),
    [flattenedOrders, pagination.limit],
  );
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mb-6 space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-medium text-gray-600">Số lần FaceID (trang)</p>
            <p className="mt-1 text-xl font-semibold text-gray-900">{totalFaceChecks}</p>
          </div>
          <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-medium text-gray-600">Số đơn hàng (trang)</p>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {displayedOrders.length > 0 ? (
                    displayedOrders.map(({ order, item, hasFaceId, faceCount }, idx) => (
                      <tr key={`${order.docCode}-${idx}`} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 text-sm text-gray-900 font-medium">{order.docCode}</td>
                ...
                <div className="flex-1 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    <p className="text-sm text-gray-700">
                      ...
                      <span className="font-medium">{flattenedOrders.length}</span> đơn hàng của trang hiện tại
                      {' • '}
