...
export default function PlatformFeeMapPage() {
  ...
  const [editingFeeMap, setEditingFeeMap] = useState<PlatformFeeMap | null>(
    null,
  );
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [deletingFeeMap, setDeletingFeeMap] = useState<PlatformFeeMap | null>(
    null,
  );
    ...
    try {
      ...
      setPagination((prev) => ({
        ...
        totalPages:
          ...
          Math.ceil((response.data.meta?.total || data.length) / prev.limit),
      }));
    } catch (error: any) {
      showToast(
        "error",
  ...
  const handleSave = async () => {
    ...
    // sanitize formData so description is string | undefined (no null)
    const payload: {
      platform?: string;
      rawFeeName?: string;
      internalCode?: string;
    ...
    try {
      setSaving(true);
      if (editingFeeMap?.id) {
        await platformFeeImportApi.updateFeeMap(editingFeeMap.id, payload);
        showToast("success", "Cập nhật map phí thành công");
      } else {
        await platformFeeImportApi.createFeeMap(payload as any);
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, rawFeeName: e.target.value })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  required
                />
      ...
      {showDeleteModal && deletingFeeMap && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <p className="mb-4">
              Bạn có chắc chắn muốn xóa map phí{" "}
              <strong>{deletingFeeMap.rawFeeName}</strong>?
            </p>
