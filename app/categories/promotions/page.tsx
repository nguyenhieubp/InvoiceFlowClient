...
const FIELD_LABELS: Record<keyof PromotionItem, string> = {
  id: 'ID',
  ...
  maPhi: 'Mã phí',
  maBoPhan: 'Mã bộ phận',
  taiKhoanChietKhau: 'Tài khoản chiết khấu',
  taiKhoanChiPhiKhuyenMai: 'Tài khoản chi phí khuyến mãi',
...
const MAIN_COLUMNS: (keyof PromotionItem)[] = [
  'maChuongTrinh',
  'tenChuongTrinh',
  'muaHangGiamGia',
  'ckTheoCS',
...
export default function PromotionsPage() {
  ...
  const [selectedColumns, setSelectedColumns] = useState<(keyof PromotionItem)[]>(
    [...MAIN_COLUMNS]
  );
  ...
  const loadPromotions = async () => {
    try {
      ...
      showToast('error', 'Lỗi khi tải danh sách chương trình khuyến mại: ' + (error.response?.data?.message || error.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleColumn = (field: keyof PromotionItem) => {
    setSelectedColumns(prev => {
      ...
      if (index > -1) {
        ...
        const allFields = Object.keys(FIELD_LABELS) as (keyof PromotionItem)[];
  ...
  const handleImport = async () => {
    ...
    try {
      setImporting(true);
      const response = await categoriesApi.importPromotionsExcel(importFile);
  ...
  const handleDelete = (promotion: PromotionItem) => {
    setDeletingPromotion(promotion);
    setShowDeleteModal(true);
  };

  const confirmDelete = async () => {
    if (!deletingPromotion?.id) return;

    try {
      setDeleting(true);
      await categoriesApi.deletePromotion(deletingPromotion.id);
      showToast('success', 'Xóa chương trình khuyến mại thành công');
      await loadPromotions();
      setShowDeleteModal(false);
      setDeletingPromotion(null);
    } catch (error: any) {
      showToast('error', 'Lỗi khi xóa chương trình khuyến mại: ' + (error.response?.data?.message || error.message));
    } finally {
      setDeleting(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingPromotion?.id) {
        await categoriesApi.updatePromotion(editingPromotion.id, formData);
        showToast('success', 'Cập nhật chương trình khuyến mại thành công');
      } else {
        await categoriesApi.createPromotion(formData);
        showToast('success', 'Tạo chương trình khuyến mại thành công');
      }
      await loadPromotions();
      ...
      showToast('error', 'Lỗi khi lưu chương trình khuyến mại: ' + (error.response?.data?.message || error.message));
    } finally {
      setSaving(false);
    }
  };

  const updateFormField = (field: keyof PromotionItem, value: any) => {
  ...
  return (
    <div className="min-h-screen bg-white relative overflow-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <h1 className="text-2xl font-bold text-gray-900">Danh mục khuyến mại</h1>
                  ...
                  {Object.entries(FIELD_LABELS)
                    ...
                    .map(([key, label]) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={selectedColumns.includes(key as keyof PromotionItem)}
                          onChange={() => toggleColumn(key as keyof PromotionItem)}
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            <div className="text-center">
              ...
              <p className="text-gray-500">Không có chương trình khuyến mại nào</p>
            </div>
          </div>
        ) : (
                  ...
                  {filteredPromotions.map((promotion, index) => (
                    <tr key={promotion.id || index} className="hover:bg-gray-50 transition-colors">
                      {visibleColumns.map((field) => (
                        ...
                        >
                          <div className="max-w-xs truncate" title={String(promotion[field] || '')}>
                            {formatValue(promotion[field])}
                          </div>
                        </td>
                      ))}
                    ...
                    <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-blue-400 transition-colors">
                      <div className="space-y-1 text-center">
                        ...
                        <div className="flex text-sm text-gray-600">
                          <label
                            htmlFor="file-upload-promotions"
                            className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500"
                          >
                            <span>Chọn file</span>
                            <input
                              id="file-upload-promotions"
                              name="file-upload-promotions"
                              type="file"
                              accept=".xlsx,.xls,.csv"
                              className="sr-only"
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-900">
                {editingPromotion ? 'Sửa chương trình khuyến mại' : 'Thêm chương trình khuyến mại mới'}
              </h2>
              <button
                onClick={() => {
                  setShowFormModal(false);
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateFormField('maChuongTrinh', e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateFormField('tenChuongTrinh', e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      />
                    </div>
                ...
                {/* Promotion Types */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b">Loại khuyến mại</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      { key: 'muaHangGiamGia', label: 'Mua hàng giảm giá' },
                      { key: 'ckTheoCS', label: 'CK theo CS' },
                      { key: 'ckVIP', label: 'CK VIP' },
                      { key: 'voucher', label: 'VOUCHER' },
                      { key: 'coupon', label: 'COUPON' },
                      { key: 'ecode', label: 'ECODE' },
                      { key: 'tangHang', label: 'Tặng hàng' },
                      { key: 'nskm', label: 'NSKM' },
                ...
                {/* Other Information */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b">Thông tin khác</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      { key: 'maPhi', label: 'Mã phí' },
                      { key: 'maBoPhan', label: 'Mã bộ phận' },
                      { key: 'taiKhoanChietKhau', label: 'Tài khoản chiết khấu' },
                      { key: 'taiKhoanChiPhiKhuyenMai', label: 'Tài khoản chi phí khuyến mãi' },
            ...
            <div className="p-6 border-t border-gray-200 flex items-center justify-end gap-3">
              ...
              <button
                onClick={handleSave}
                disabled={saving || !formData.maChuongTrinh}
                className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2"
              >
                {saving ? (
                  <>
      ...
      {showDeleteModal && deletingPromotion && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-4">Xác nhận xóa</h2>
              <p className="text-gray-700 mb-6">
                Bạn có chắc chắn muốn xóa chương trình khuyến mại <strong>{deletingPromotion.maChuongTrinh} - {deletingPromotion.tenChuongTrinh}</strong>?
