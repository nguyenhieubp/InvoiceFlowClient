...
const FIELD_LABELS: Record<keyof ProductItem, string> = {
  id: 'ID',
  maNhanHieu: 'Mã nhãn hiệu',
  loai: 'Loại',
  lop: 'Lớp',
  nhom: 'Nhóm',
  line: 'Line',
  maERP: 'Mã ERP',
  maVatTuNhaCungCap: 'Mã Vật tư nhà cung cấp',
  maVatTu: 'Mã vật tư',
  tenVatTu: 'Tên vật tư',
  tenKhac: 'Tên khác',
  tenHD: 'Tên (HD)',
  ...
  dvt: 'Đvt',
  nhieuDvt: 'Nhiều Đvt',
  theoDoiTonKho: 'Theo dõi tồn kho',
  theoDoiLo: 'Theo dõi lô',
  theoDoiKiemKe: 'Theo dõi kiểm kê',
  theoDoiSerial: 'Theo dõi serial',
  cachTinhGia: 'Cách tính giá',
  ...
  maKho: 'Mã kho',
  maViTri: 'Mã vị trí',
  thueGiaTriGiaTang: 'Thuế giá trị gia tăng',
  thueNhapKhau: 'Thuế nhập khẩu',
  ...
  kieuLo: 'Kiểu lô',
  cachXuat: 'Cách xuất',
  vongDoiSP: 'Vòng đời sp (số ngày)',
  tgBaoHanh: 'TG bảo hành (số ngày)',
  choPhepTaoLoNgayKhiNhap: 'Cho phép tạo lô ngay khi nhập',
  abc: 'ABC',
  soLuongTonToiThieu: 'Số lượng tồn tối thiểu',
  soLuongTonToiDa: 'Số lượng tồn tối đa',
  theTich: 'Thể tích',
  donViTinhTheTich: 'Đơn vị tính',
  khoiLuong: 'Khối lượng',
  donViTinhKhoiLuong: 'Đơn vị tính',
  giaDichVu: 'Giá dịch vụ',
  loaiHinhDichVu: 'Loại hình dịch vụ',
  maVatTuGoc: 'Mã vật tư gốc',
  ...
  tyLeTrichGiaVon: 'Tỷ lệ trích giá vốn',
...
// Các fields chính để hiển thị trong bảng (có thể điều chỉnh)
const MAIN_COLUMNS: (keyof ProductItem)[] = [
  'maNhanHieu',
  'maVatTu',
  'tenVatTu',
  'barcode',
...
export default function ProductsPage() {
  ...
  const loadProducts = async () => {
    try {
      ...
      showToast('error', 'Lỗi khi tải danh sách sản phẩm: ' + (error.response?.data?.message || error.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleColumn = (field: keyof ProductItem) => {
    setSelectedColumns(prev => {
      ...
      if (index > -1) {
        // Nếu đã có thì xóa, nhưng giữ thứ tự của các cột khác
        return prev.filter(col => col !== field);
      } else {
        // Nếu chưa có thì thêm vào cuối, nhưng cần tìm vị trí phù hợp trong danh sách gốc
        const allFields = Object.keys(FIELD_LABELS) as (keyof ProductItem)[];
        const fieldIndex = allFields.indexOf(field);
        
        // Tìm vị trí chèn dựa trên thứ tự trong FIELD_LABELS
  ...
  const formatValue = (value: any): React.ReactNode => {
    ...
    if (typeof value === 'string') {
      // Kiểm tra nếu là date string (ISO format)
  ...
  const handleImport = async () => {
    ...
    try {
      setImporting(true);
      const response = await categoriesApi.importExcel(importFile);
  ...
  const confirmDelete = async () => {
    ...
    try {
      setDeleting(true);
      await categoriesApi.deleteProduct(deletingProduct.id);
      showToast('success', 'Xóa sản phẩm thành công');
      await loadProducts();
      setShowDeleteModal(false);
      setDeletingProduct(null);
    } catch (error: any) {
      showToast('error', 'Lỗi khi xóa sản phẩm: ' + (error.response?.data?.message || error.message));
    } finally {
      setDeleting(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingProduct?.id) {
        await categoriesApi.updateProduct(editingProduct.id, formData);
        showToast('success', 'Cập nhật sản phẩm thành công');
      } else {
        await categoriesApi.createProduct(formData);
      ...
      showToast('error', 'Lỗi khi lưu sản phẩm: ' + (error.response?.data?.message || error.message));
    } finally {
      setSaving(false);
    }
  };

  const updateFormField = (field: keyof ProductItem, value: any) => {
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <h1 className="text-2xl font-bold text-gray-900">Danh mục hàng hóa vật tư</h1>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-h-64 overflow-y-auto">
                  {Object.entries(FIELD_LABELS)
                    ...
                    .map(([key, label]) => (
                      ...
                      <input
                        type="checkbox"
                        checked={selectedColumns.includes(key as keyof ProductItem)}
                        onChange={() => toggleColumn(key as keyof ProductItem)}
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Tìm kiếm theo mã vật tư, tên vật tư, barcode, mã ERP..."
                value={searchQuery}
                onChange={(e) => {
                  ...
                  {filteredProducts.map((product, index) => (
                    <tr key={product.id || index} className="hover:bg-gray-50 transition-colors">
                      {visibleColumns.map((field) => (
                        ...
                        >
                          <div className="max-w-xs truncate" title={String(product[field] || '')}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      { key: 'maVatTu', required: true },
                      { key: 'tenVatTu', required: true },
                      { key: 'maNhanHieu' },
                      { key: 'loai' },
                      { key: 'lop' },
                      { key: 'nhom' },
                      { key: 'line' },
                      { key: 'maERP' },
                      { key: 'maVatTuNhaCungCap' },
                      { key: 'tenKhac' },
                      { key: 'tenHD' },
                      { key: 'barcode' },
                      { key: 'dvt' },
                      { key: 'trangThai', type: 'select', options: [
                      ...
                      <div key={field.key}>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          {FIELD_LABELS[field.key as keyof ProductItem]} {field.required && <span className="text-red-500">*</span>}
                        </label>
                        {field.type === 'select' ? (
                          <select
                            value={formData[field.key as keyof ProductItem] as string || ''}
                            onChange={(e) => updateFormField(field.key as keyof ProductItem, e.target.value)}
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b">Phân loại</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {['loaiVatTu', 'loaiHang', 'nhomGia', 'maKho', 'maViTri', 'kieuLo', 'cachXuat', 'cachTinhGia', 'abc'].map((key) => (
                      <div key={key}>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{FIELD_LABELS[key as keyof ProductItem]}</label>
                        <input
                          type="text"
                          value={(formData[key as keyof ProductItem] as string) || ''}
                          onChange={(e) => updateFormField(key as keyof ProductItem, e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        />
                      </div>
                    ))}
                ...
                {/* Tracking Section */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b">Theo dõi</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {['nhieuDvt', 'theoDoiTonKho', 'theoDoiLo', 'theoDoiKiemKe', 'theoDoiSerial', 'suaTkVatTu', 'choPhepTaoLoNgayKhiNhap'].map((key) => (
                      <div key={key} className="flex items-center">
                        <input
                          type="checkbox"
                          checked={formData[key as keyof ProductItem] as boolean || false}
                          onChange={(e) => updateFormField(key as keyof ProductItem, e.target.checked)}
                          className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                        />
                        <label className="ml-2 block text-sm text-gray-700">{FIELD_LABELS[key as keyof ProductItem]}</label>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Pricing Section */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b">Giá & Thuế</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      { key: 'thueGiaTriGiaTang', type: 'number' },
                      { key: 'thueNhapKhau', type: 'number' },
                      { key: 'vongDoiSP', type: 'number' },
                      { key: 'tgBaoHanh', type: 'number' },
                      { key: 'giaDichVu', type: 'number' },
                      { key: 'loaiHinhDichVu' },
                      { key: 'tyLeTrichGiaVon', type: 'number' },
                    ].map((field) => (
                      <div key={field.key}>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{FIELD_LABELS[field.key as keyof ProductItem]}</label>
                        <input
                          type={field.type || 'text'}
                          value={(formData[field.key as keyof ProductItem] as string | number) || ''}
                          onChange={(e) => {
                            const value = field.type === 'number' ? (e.target.value ? parseFloat(e.target.value) : undefined) : e.target.value;
                            updateFormField(field.key as keyof ProductItem, value);
                          }}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        />
                      </div>
                ...
                {/* Accounting Section */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b">Tài khoản</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {['tkVatTu', 'tkGiaVonBanBuon', 'tkDoanhThuBanBuon', 'tkDoanhThuNoiBo', 'tkHangBanTraLai', 'tkDaiLy', 'tkSanPhamDoDang', 'tkChenhLechGiaVon', 'tkChietKhau', 'tkChiPhiKhuyenMai', 'tkGiaVonBanLe', 'tkDoanhThuBanLe', 'tkChiPhiKhauHaoCCDC', 'tkChiPhiKhauHaoTSDC', 'tkDoanhThuHangNo', 'tkGiaVonHangNo', 'tkVatTuHangNo'].map((key) => (
                      <div key={key}>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{FIELD_LABELS[key as keyof ProductItem]}</label>
                        <input
                          type="text"
                ...
                {/* Inventory Section */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b">Tồn kho</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      { key: 'soLuongTonToiThieu', type: 'number' },
                      { key: 'soLuongTonToiDa', type: 'number' },
                      { key: 'theTich', type: 'number' },
                      { key: 'donViTinhTheTich' },
                      { key: 'khoiLuong', type: 'number' },
                      { key: 'donViTinhKhoiLuong' },
                    ].map((field) => (
                      <div key={field.key}>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{FIELD_LABELS[field.key as keyof ProductItem]}</label>
                        <input
                ...
                {/* Other Section */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b">Khác</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {['maVatTuGoc', 'danhSachDonVi', 'maNhaCungCap', 'nguoiTao', 'nguoiSua'].map((key) => (
                      <div key={key}>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{FIELD_LABELS[key as keyof ProductItem]}</label>
                        <input
                          type="text"
            ...
            <div className="p-6 border-t border-gray-200 flex items-center justify-end gap-3">
              ...
              <button
                onClick={handleSave}
                disabled={saving || !formData.maVatTu || !formData.tenVatTu}
                className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2"
              >
                {saving ? (
                  <>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              ...
              <p className="text-gray-700 mb-6">
                Bạn có chắc chắn muốn xóa sản phẩm <strong>{deletingProduct.maVatTu} - {deletingProduct.tenVatTu}</strong>?
