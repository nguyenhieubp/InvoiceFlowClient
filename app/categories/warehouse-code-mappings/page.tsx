...
interface WarehouseCodeMapping {
  id?: string;
  maCu: string;
  maMoi: string;
  trangThai?: string;
...
const FIELD_LABELS: Record<keyof WarehouseCodeMapping, string> = {
  id: 'ID',
  maCu: 'Mã cũ',
  maMoi: 'Mã mới',
...
const MAIN_COLUMNS: (keyof WarehouseCodeMapping)[] = [
  'maCu',
  'maMoi',
  'trangThai',
];

export default function WarehouseCodeMappingsPage() {
  const [mappings, setMappings] = useState<WarehouseCodeMapping[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedColumns, setSelectedColumns] = useState<(keyof WarehouseCodeMapping)[]>(
    [...MAIN_COLUMNS]
  );
  ...
  const [editingMapping, setEditingMapping] = useState<WarehouseCodeMapping | null>(null);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [deletingMapping, setDeletingMapping] = useState<WarehouseCodeMapping | null>(null);
  const [formData, setFormData] = useState<Partial<WarehouseCodeMapping>>({});
  ...
  const loadMappings = async () => {
    try {
      setLoading(true);
      const response = await categoriesApi.getWarehouseCodeMappings({
      ...
      showToast('error', 'Lỗi khi tải danh sách mapping: ' + (error.response?.data?.message || error.message));
    } finally {
      setLoading(false);
    }
  };

  const toggleColumn = (field: keyof WarehouseCodeMapping) => {
    setSelectedColumns(prev => {
      const index = prev.indexOf(field);
      if (index > -1) {
        return prev.filter(col => col !== field);
      } else {
        const allFields = Object.keys(FIELD_LABELS) as (keyof WarehouseCodeMapping)[];
  ...
  const handleImport = async () => {
    ...
    try {
      setImporting(true);
      const response = await categoriesApi.importWarehouseCodeMappingsExcel(importFile);
  ...
  const handleEdit = (mapping: WarehouseCodeMapping) => {
    setEditingMapping(mapping);
    setFormData({ ...mapping });
    setShowFormModal(true);
  };

  const handleDelete = (mapping: WarehouseCodeMapping) => {
    setDeletingMapping(mapping);
    setShowDeleteModal(true);
  };

  const confirmDelete = async () => {
    if (!deletingMapping?.id) return;

    try {
      setDeleting(true);
      await categoriesApi.deleteWarehouseCodeMapping(deletingMapping.id);
      showToast('success', 'Xóa mapping thành công');
      await loadMappings();
      setShowDeleteModal(false);
      setDeletingMapping(null);
    } catch (error: any) {
      showToast('error', 'Lỗi khi xóa mapping: ' + (error.response?.data?.message || error.message));
    } finally {
      setDeleting(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingMapping?.id) {
        await categoriesApi.updateWarehouseCodeMapping(editingMapping.id, formData);
        showToast('success', 'Cập nhật mapping thành công');
      } else {
        await categoriesApi.createWarehouseCodeMapping(formData);
        showToast('success', 'Tạo mapping thành công');
      }
      await loadMappings();
      ...
      showToast('error', 'Lỗi khi lưu mapping: ' + (error.response?.data?.message || error.message));
    } finally {
      setSaving(false);
    }
  };

  const updateFormField = (field: keyof WarehouseCodeMapping, value: any) => {
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <h1 className="text-2xl font-bold text-gray-900">Danh mục kho - Mapping mã</h1>
                  ...
                  {Object.entries(FIELD_LABELS)
                    ...
                    .map(([key, label]) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={selectedColumns.includes(key as keyof WarehouseCodeMapping)}
                          onChange={() => toggleColumn(key as keyof WarehouseCodeMapping)}
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Tìm kiếm theo mã cũ, mã mới..."
                value={searchQuery}
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            <div className="text-center">
              ...
              <p className="text-gray-500">Không có mapping nào</p>
            </div>
          </div>
        ) : (
                  ...
                  {mappings.map((mapping, index) => (
                    <tr key={mapping.id || index} className="hover:bg-gray-50 transition-colors">
                      {visibleColumns.map((field) => (
                        ...
                        >
                          <div className="max-w-xs truncate" title={String(mapping[field] || '')}>
                            {formatValue(mapping[field])}
                          </div>
                        </td>
                      ))}
                ...
                <div className="space-y-4">
                    ...
                    <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-blue-400 transition-colors">
                      <div className="space-y-1 text-center">
                        ...
                        <div className="flex text-sm text-gray-600">
                          <label
                            htmlFor="file-upload-mappings"
                            className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500"
                          >
                            <span>Chọn file</span>
                            <input
                              id="file-upload-mappings"
                              name="file-upload-mappings"
                              type="file"
                              accept=".xlsx,.xls,.csv"
                              className="sr-only"
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <ul className="text-sm text-blue-800 space-y-1 list-disc list-inside">
                      ...
                      <li>Các cột trong file: <strong>&quot;Cũ&quot;</strong> (hoặc &quot;Mã cũ&quot;) và <strong>&quot;Mới&quot;</strong> (hoặc &quot;Mã mới&quot;)</li>
                      <li>Mã cũ và Mã mới là bắt buộc. Nếu mã cũ đã tồn tại, dữ liệu sẽ được xóa và tạo mới</li>
                      <li>Ví dụ format: Cột 1: &quot;Cũ&quot; (BAS1, LAS1), Cột 2: &quot;Mới&quot; (BMH01, LMH01)</li>
                    </ul>
                  </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-900">
                {editingMapping ? 'Sửa mapping' : 'Thêm mapping mới'}
              </h2>
              <button
                onClick={() => {
                  setShowFormModal(false);
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    { key: 'maCu', label: 'Mã cũ', required: true },
                    { key: 'maMoi', label: 'Mã mới', required: true },
                    ...
                    <div key={field.key}>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        {FIELD_LABELS[field.key as keyof WarehouseCodeMapping]} {field.required && <span className="text-red-500">*</span>}
                      </label>
                      {field.type === 'select' ? (
                        <select
                          value={formData[field.key as keyof WarehouseCodeMapping] as string || ''}
                          onChange={(e) => updateFormField(field.key as keyof WarehouseCodeMapping, e.target.value)}
            ...
            <div className="p-6 border-t border-gray-200 flex items-center justify-end gap-3">
              ...
              <button
                onClick={handleSave}
                disabled={saving || !formData.maCu || !formData.maMoi}
                className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2"
              >
                {saving ? (
                  <>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              ...
              <p className="text-gray-700 mb-6">
                Bạn có chắc chắn muốn xóa mapping <strong>{deletingMapping.maCu} → {deletingMapping.maMoi}</strong>?
