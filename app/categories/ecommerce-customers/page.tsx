...
export default function EcommerceCustomersPage() {
    ...
    const [selectedColumns, setSelectedColumns] = useState<(keyof EcommerceCustomer)[]>(
        [...MAIN_COLUMNS]
    );
    ...
    const [editingCustomer, setEditingCustomer] = useState<EcommerceCustomer | null>(null);
    const [showDeleteModal, setShowDeleteModal] = useState(false);
    const [deletingCustomer, setDeletingCustomer] = useState<EcommerceCustomer | null>(null);
    const [formData, setFormData] = useState<Partial<EcommerceCustomer>>({});
    ...
    const loadCustomers = async () => {
        try {
            setLoading(true);
            const response = await categoriesApi.getEcommerceCustomers({
    ...
    const toggleColumn = (field: keyof EcommerceCustomer) => {
        setSelectedColumns(prev => {
            const index = prev.indexOf(field);
            if (index > -1) {
                return prev.filter(col => col !== field);
            } else {
                const allFields = Object.keys(FIELD_LABELS) as (keyof EcommerceCustomer)[];
    ...
    const handleImport = async () => {
        ...
        try {
            setImporting(true);
            const response = await categoriesApi.importEcommerceCustomersExcel(importFile);
    ...
    const handleEdit = (customer: EcommerceCustomer) => {
        setEditingCustomer(customer);
        setFormData({ ...customer });
        setShowFormModal(true);
    };

    const handleDelete = (customer: EcommerceCustomer) => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            setDeleting(true);
            await categoriesApi.deleteEcommerceCustomer(deletingCustomer.id);
    ...
    const handleSave = async () => {
        try {
            ...
            if (editingCustomer?.id) {
                await categoriesApi.updateEcommerceCustomer(editingCustomer.id, formData);
                showToast('success', 'Cập nhật thành công');
            } else {
                await categoriesApi.createEcommerceCustomer(formData);
            ...
            showToast('error', 'Lỗi khi lưu: ' + (error.response?.data?.message || error.message));
        } finally {
            setSaving(false);
        }
    };

    const updateFormField = (field: keyof EcommerceCustomer, value: any) => {
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
                    <div className="flex flex-col gap-4">
                        <div className="flex items-center justify-between flex-wrap gap-3">
                            <h1 className="text-2xl font-bold text-gray-900">Danh mục khách hàng sàn TMDT</h1>
                                    ...
                                    {Object.entries(FIELD_LABELS)
                                        ...
                                        .map(([key, label]) => (
                                            ...
                                            >
                                                <input
                                                    type="checkbox"
                                                    checked={selectedColumns.includes(key as keyof EcommerceCustomer)}
                                                    onChange={() => toggleColumn(key as keyof EcommerceCustomer)}
                        ...
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder="Tìm kiếm theo Brand, Customer Code..."
                                value={searchQuery}
                                    ...
                                    {customers.map((customer, index) => (
                                        <tr key={customer.id || index} className="hover:bg-gray-50 transition-colors">
                                            {visibleColumns.map((field) => (
                                                ...
                                                >
                                                    <div className="max-w-xs truncate" title={String(customer[field] || '')}>
                                                        {formatValue(customer[field])}
                                                    </div>
                                                </td>
                                            ))}
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="p-6">
                            ...
                            {!importResult ? (
                                        ...
                                        <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-blue-400 transition-colors">
                                            <div className="space-y-1 text-center">
                                                ...
                                                <div className="flex text-sm text-gray-600">
                                                    <label
                                                        htmlFor="file-upload-ecommerce"
                                                        className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500"
                                                    >
                                                        <span>Chọn file</span>
                                                        <input
                                                            id="file-upload-ecommerce"
                                                            name="file-upload-ecommerce"
                                                            type="file"
                                                            accept=".xlsx,.xls,.csv"
                                                            className="sr-only"
                                ...
                                <div className="space-y-4">
                                    <div className={`p-4 rounded-lg border-2 ${importResult.success > 0
                                        ? 'bg-green-50 border-green-300'
                                        : 'bg-red-50 border-red-300'
                                        }`}>
                                        <h3 className={`text-lg font-semibold mb-2 ${importResult.success > 0 ? 'text-green-900' : 'text-red-900'
                                            }`}>
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
                            <h2 className="text-xl font-bold text-gray-900">
                                {editingCustomer ? 'Sửa thông tin' : 'Thêm mới'}
                            </h2>
                            <button
                                onClick={() => {
                                    setShowFormModal(false);
                                    ...
                                    {[
                                        ...
                                        <div key={field.key}>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                                {FIELD_LABELS[field.key as keyof EcommerceCustomer]} {field.required && <span className="text-red-500">*</span>}
                                            </label>
                                            {field.type === 'select' ? (
                                                <select
                                                    value={formData[field.key as keyof EcommerceCustomer] as string || ''}
                                                    onChange={(e) => updateFormField(field.key as keyof EcommerceCustomer, e.target.value)}
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                        <div className="p-6">
                            ...
                            <p className="text-sm text-gray-500 text-center mb-6">
                                Bạn có chắc chắn muốn xóa <strong>{deletingCustomer?.customerCode}</strong>? Hành động này không thể hoàn tác.
                            </p>
